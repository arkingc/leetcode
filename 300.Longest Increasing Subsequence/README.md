...
当处理1时，这个升序序列被终止，因此，1可能开启一个新的升序序列，将5修改成1：

```
S = [1,6,7]
```

注意，虽然S并不是正确的升序序列，但是最长升序序列的长度3这个性质依然保留。当处理2时，替换6：
...
因为只需遍历一遍数组，所以可以直接在nums的前部进行修改，作为S，从而不需要而外的空间
