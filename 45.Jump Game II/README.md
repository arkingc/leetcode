...
* 还有一点需要注意。对于每个点，应该从该点所能跳跃的最大步开始尝试，对于点i，设该位置能跳跃的最大步为nums[i]，那么应该先尝试跳跃nums[i]步，如果不能到达终点，那么判断点`i+nums[i]`是否已经加入到队列中，如果没有则加入，**如果已经加入，那么停止尝试跳`nums[i] - 1`,`nums[i] - 2`,...,`1`步，因为从i跳nums[i]步不能到达终点，那么跳更小的步数肯定也不能到达。同时，如果`i+nums[i]`已经添加到了队列中，那么`i + nums[i] - 1`，`i + nums[i] - 2`,...,`i + 1`肯定也已经添加到了队列中**
